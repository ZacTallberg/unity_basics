...
namespace HexasphereGrid {
	...
	public partial class Hexasphere : MonoBehaviour {
		...
		bool currentExtruded, currentInvertedMode, currentWireframeColorFromTile, currentSmartEdges;
		...
		bool mouseIsOver, mouseStartedDragging;
		...
		void Start () {
			if(cameraOrigin == null) cameraOrigin = GameObject.Find("cameraOrigin");
			RegisterVRPointers ();
		}
		...
		void CheckUserInteractionNormalMode () {
			...
			if (_rotationEnabled) {
				if (leftMouseButtonClick) {
					...
					#endif
					mouseDragStartLocalPosition = cameraOrigin.transform.InverseTransformPoint (position);
					...
					#else
					Vector3 localPos = cameraOrigin.transform.InverseTransformPoint (position);
					...
					if (localPos != mouseDragStartLocalPosition) {
						...
						if(reverseDragDirection == true)
						{
							angle *= -1;
						}
						Quaternion rot = Quaternion.AngleAxis (angle, Vector3.Cross (mouseDragStartLocalPosition, localPos));  

						//makes the rotation here
						if (_rotationSpeed < 1f) {
							Quaternion newRot = cameraOrigin.transform.rotation * rot;
							cameraOrigin.transform.rotation = Quaternion.Slerp (cameraOrigin.transform.rotation, newRot, _rotationSpeed);
							//Quaternion newRot = transform.rotation * rot;
							//transform.rotation = Quaternion.Slerp (transform.rotation, newRot, _rotationSpeed);
						} else {
							cameraOrigin.transform.rotation *= rot;
			...
			if (leftMouseButtonRelease && (Application.isMobilePlatform || (!hasDragged && !mouseStartedDragging)) && OnTileClick != null) {
		...
		void KeepStraight () {
			...
			Vector3 v3 = Vector3.ProjectOnPlane (cameraOrigin.transform.up, v2);
			//Vector3 v3 = Vector3.ProjectOnPlane (transform.up, v2);
			float angle2 = SignedAngleBetween (Camera.main.transform.up, v3, v2);
			cameraOrigin.transform.Rotate (v2, -angle2, Space.World);
			...
			if (angle2 > 0 && angle2 < _rotationAxisVerticalThreshold) {
				cameraOrigin.transform.Rotate(Camera.main.transform.right, _rotationAxisVerticalThreshold - angle2, Space.World);
				//transform.Rotate (Camera.main.transform.right, _rotationAxisVerticalThreshold - angle2, Space.World);
			} else if (angle2 > -_rotationAxisVerticalThreshold && angle2 < 0) {
				cameraOrigin.transform.Rotate (Camera.main.transform.right, _rotationAxisVerticalThreshold + angle2, Space.World);
				//transform.Rotate (Camera.main.transform.right, _rotationAxisVerticalThreshold + angle2, Space.World);
			}
			if (angle2 > (180 - _rotationAxisVerticalThreshold) && angle2 < 180) {
				cameraOrigin.transform.Rotate (Camera.main.transform.right, (180 - _rotationAxisVerticalThreshold) - angle2, Space.World);
				//transform.Rotate (Camera.main.transform.right, (180 - _rotationAxisVerticalThreshold) - angle2, Space.World);
		...
		public delegate void testInt(int testing);
		public static event testInt testNow;
		void CheckMousePosNormalMode (out Vector3 position, out Ray ray) {
			...
			if (_highlightEnabled || OnTileMouseOver != null || (!Application.isPlaying && useEditorRay)) {
				...
				if (_extruded && _raycast3D) {
					...
					//Vector3 localPosition = cameraOrigin.transform.InverseTransformPoint (position);
		...
		public void UpdateMaterialProperties () {
			...
			if (tiles == null || currentDivisions != _numDivisions || currentStyle != _style || currentTextureSize != _tileTextureSize || currentDefaultShadedColor != _defaultShadedColor || currentExtruded != _extruded || _rotationShift != currentRotationShift || _invertedMode != currentInvertedMode) {
				Generate ();
			} else {
		...
		void UpdateTransparencyMode () {
			...
			if (_transparencyTiles < 1) {
				if (_tileShadedFrameMatExtrusion != null) {
					if (_transparencyTiles < 1f && _transparencyCull) {
		...
		void UpdateLightingMode () {
			if (_tileShadedFrameMatExtrusion != null) {
				if (_lighting) {
					_tileShadedFrameMatExtrusion.EnableKeyword ("HEXA_LIT");
				} else {
					_tileShadedFrameMatExtrusion.DisableKeyword ("HEXA_LIT");
				}
			}
			if (_tileShadedFrameMatNoExtrusion != null) {
				if (_lighting) {
					_tileShadedFrameMatNoExtrusion.EnableKeyword ("HEXA_LIT");
				} else {
					_tileShadedFrameMatNoExtrusion.DisableKeyword ("HEXA_LIT");
				}
			}
		}
		...
		void UpdateTransparencyMat (Material mat) {
			...
			if (_transparent) {
				...
				mat.SetInt ("_ZWrite", 1); // kept to 1 to avoid clipping issues
		...
		void UpdateMeshRenderersShadowSupport () {
			...
			MeshRenderer[] rr = transform.GetComponentsInChildren<MeshRenderer> ();
			for (int k = 0; k < rr.Length; k++) {
				if (rr [k] != null) {
					rr [k].receiveShadows = _receiveShadows;
					if (_lighting) { // Shadow casting generates horrible shadow acné so this feature is left out for now
		...
		void GenerateTileMesh (int tileIndex, Material mat) {
			if (tilesRoot == null) {
				...
				//Add the MeshMerger script to the tilesRoot
				//tilesRoot.gameObject.AddComponent<MeshMerger>();
