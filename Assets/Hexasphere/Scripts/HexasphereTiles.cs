...
namespace HexasphereGrid {
	...
	public partial class Hexasphere: MonoBehaviour {
		...
		public bool SetTileMaterial (int tileIndex, Material mat, bool temporary = false) {
			...
			if (highlightMaterial != null && tile == lastHighlightedTile) {
				...
				if (tile.tempMat != null) {
					...
					if (tile.tempMat.HasProperty ("_MainTex") && highlightMaterial.HasProperty ("_MainTex")) {
					...
					if (tile.customMat.HasProperty ("_MainTex") && highlightMaterial.HasProperty ("_MainTex")) {
		...
		/// </summary>
		public bool SetTileCrossCost (int tileIndex, int crossCost) {
			if (tileIndex < 0 || tileIndex >= tiles.Length)
				return false;
		...
		/// </summary>
		public int GetTileCrossCost (int tileIndex) {
			if (tileIndex < 0 || tileIndex >= tiles.Length)
				return 0;
		...
		public Vector3 GetTileCenter (int tileIndex, bool worldSpace = true, bool includeExtrusion = true) {
			if (tileIndex < 0 || tileIndex >= tiles.Length)
				return Misc.Vector3zero;
			Tile tile = tiles [tileIndex];
			Vector3 v = tile.center;
			if (includeExtrusion) {
		...
		/// <summary>
		/// Returns the tile latitude and longitude in a Vector2 field. LatLon is only available if the methods ApplyHeightMap() or ApplyColors() have been called before.
		/// </summary>
		...
		/// <summary>
		/// Returns the UV texture coordinate of the tile center (mapped to a 2D texture) in a Vector2 field. LatLon is only available if the methods ApplyHeightMap() or ApplyColors() have been called before.
		/// </summary>
		/// <returns>The tile lat lon.</returns>
		/// <param name="tileIndex">Tile index.</param>
