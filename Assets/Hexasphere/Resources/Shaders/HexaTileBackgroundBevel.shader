 		...
 		Pass {
	        ...
	        Offset 2, 2
    			...
    			void Extrude(v2g p0, v2g p1, float extrusion, fixed4 color, float3 norm0, float3 norm1, inout TriangleStream<g2f> outputStream) {
    			    ...
    			    // First side triangle
    			    float3 v = p1.pos.xyz * extrusion;
                	tri.pos   = UnityObjectToClipPos(float4(v, p1.pos.w));
                	...
                	tri.pos   = UnityObjectToClipPos(float4(v, p0.pos.w));
    	            ...
    	            tri.norm  = tri.axisNorm = UnityObjectToWorldNormal(v - gPos);
                	TRANSFER_SHADOW(tri);
                	outputStream.Append(tri);
				...
				void geom(triangle v2g input[3], inout TriangleStream<g2f> outputStream) {
    	            ...
    	            topFace.uv = float4(input[1].uv.xyz, 1.0);
      				worldPos = mul(unity_ObjectToWorld, v).xyz;
